...
class MeteredAudioSourceNode {
  ...
  private let meterUpdateQueue = DispatchQueue(
    label: "\(Constants.Namespace).audioOutput.metering", qos: .userInteractive)
  ...
  init(format: AVAudioFormat, renderBlock: @escaping AVAudioSourceNodeRenderBlock) {
    ...
    sourceNode = AVAudioSourceNode(format: format) {
      [weak self] (isSilence, timestamp, frameCount, outputBuffer) -> OSStatus in
  ...
  private func calculateMeters(
    outputBuffer: UnsafeMutablePointer<AudioBufferList>, frameCount: AVAudioFrameCount
  ) {
